...
function commandHandlers(command) {
	if (command == 'trello_start') {
		trelloIsRunning = true;
	} else if (command == 'trello_stop') {
		trelloIsRunning = false;
	...
	} else if (command === 'trello_javascript') {
		python = false;
		javascript = true;
		console.log('Calling jsView');
		$('#editorspane').css('grid-template-columns', '100% 0% 0%');
	} else if (command === 'trello_python') {
		python = true;
		javascript = false;
		$('#editorspane').css('grid-template-columns', '0% 0% 100%');
	} else if (command === 'trello_full') {
		python = true;
		javascript = true;
		$('#editorspane').css('grid-template-columns', '49.75% 0.5% 49.75%');
	} else if (command == 'trello_run') {
		runPython();
		runJavascript();
	} else if (command == 'trello_run_py') {
		runPython();
	} else if (command == 'trello_run_js') {
		runJavascript();
	} else if (command == 'trello_zulip') {
		runZulip();
	} else if (command == 'trello_clear') {
		editor2.setValue('');
		$('#jsfile').html('index.js');
		$('#pyfile').html('index.py');
		editor1.setValue('');
	} else if (command == 'trello_redo') {
		_undo();
	} else if (command == 'trello_redo') {
		_redo();
	} else if (command == 'trello_add_closing_bracket') {
	}
}
