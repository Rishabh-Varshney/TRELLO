...
var known_commands = [
  ...
  { id: "trello_start", text: "trello start" },
  { id: "trello_stop", text: "trello stop" },
  { id: "trello_javascript", text: "trello javascript" },
  { id: "trello_python", text: "trello python" },
  { id: "trello_full", text: "trello full" },

  { id: "dexter_clear", text: "dexter clear" },

  // { id: 'trello_run', text: 'trello run code' },
  // { id: 'trello_run', text: 'code chala do bhai' },
  { id: "trello_run_py", text: "are bhai bhai bhai" },
  { id: "trello_run_py", text: "trello run python" },
  { id: "trello_run_js", text: "trello run javascript" },
  { id: "trello_zulip", text: "zulip pe bhej do" },
  { id: "trello_zulip", text: "trello save" },

  { id: "trello_undo", text: "trello undo" },
  { id: "trello_redo", text: "trello redo" },

  { id: "trello_add_closing_bracket", text: "trello add closing bracket" },
];
...
function setUpNlp() {
  ...
  // declare_integer
  {
    ...
    let declare_integer_var_name = new Bravey.StringEntityRecognizer(
      "declare_integer_var_name"
    );
    ...
    let declare_integer_var_value = new Bravey.NumberEntityRecognizer(
      "declare_integer_var_value"
    );
  ...
  //create_function
  {
    ...
    let create_function_name = new Bravey.StringEntityRecognizer(
    ...
    let create_function_argument = new Bravey.StringEntityRecognizer(
  ...
  // declare_string
  {
    ...
    let declare_string_var_name = new Bravey.StringEntityRecognizer(
      "declare_string_var_name"
    );
    for (let each of allowed_variable_names) {
    ...
    let declare_string_var_value = new Bravey.NumberEntityRecognizer(
      "declare_string_var_value"
    );
  ...
  // create loop
  {
    ...
    let create_loop_counts = new Bravey.NumberEntityRecognizer(
      "create_loop_counts"
    );
  ...
  // if_else
  {
    ...
    nlp.addDocument("if alpha equals one or beta equals one", "if_condition", {
      fromFullSentence: true,
      expandIntent: true,
    });

    nlp.addDocument("if alpha equals one and x equals zero", "if_condition", {
      fromFullSentence: true,
      expandIntent: true,
    });
  ...
  // arithmetic operations
  {
    ...
    let arithmetc_operator = new Bravey.StringEntityRecognizer(
      "arithmetic_operator"
    );
    ...
    // )
    nlp.addDocument("store sum of x and 3 in alpha", "arithmetic_operation", {
      fromFullSentence: true,
      expandIntent: true,
    });
  ...
  for (let command of known_commands) {
    nlp.addDocument(command.text, command.id, {
      fromFullSentence: true,
      expandIntent: true,
    });
  }
  ...
  // print function
  {
    ...
    // train with some examples
    nlp.addDocument("print {argument}", "print");

    nlp.addDocument("print {number}", "print");
